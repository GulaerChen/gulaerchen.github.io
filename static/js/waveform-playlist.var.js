	  ...
	  _createClass(_class, [{
	    ...
	    value: function load() {
	      ...
	      return new Promise(function (resolve, reject) {
			...
			var file_name = _this2.src.split('_')
			
	        xhr.open('GET', _this2.src, true);
	        xhr.responseType = 'arraybuffer';
	        xhr.send();
			document.getElementById('demolist').style.pointerEvents = 'none'; // let user not to click other demo song
	        xhr.addEventListener('progress', function (e) {
	        	if (file_name[file_name.length-1] == "mix.wav"){
	        		$("#overlay").css({"display":"block"});
		        	if (e.lengthComputable) {
		        	    var percentComplete = e.loaded / e.total;
		        	    $('#spinner_text').text('Downloading 2 tracks...... '+Math.round(percentComplete*100)+'%');
		        	    if (Math.round(percentComplete*100) == 100)
		        	    {
		        	    	$("#overlay").css({"display":"none"});
		        	    	document.getElementById('demolist').style.pointerEvents = 'auto';
		        	    }
					}
	        	}
	  ...
	  _createClass(_class, [{
	    ...
	    */
	    ...
	    value: function renderControls(data) {
	      ...
	      var muteClass = data.muted ? '.btn-secondary.active' : '.btn-light';
	      var soloClass = data.soloed ? '.btn-secondary.active' : '.btn-light';
	      ...
	      }, [(0, _h2.default)('header', [this.name]), (0, _h2.default)('div.btn-group.btn-group-toggle', [(0, _h2.default)('button.btn' + muteClass, {
	      ...
	      }, ['Mute']), (0, _h2.default)('button.btn' + soloClass, {
	        onclick: function onclick() {
	        	if (data.muted == true)
	        	{
	        		_this2.ee.emit('mute', _this2);
